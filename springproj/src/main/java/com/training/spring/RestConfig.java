...
public class RestConfig {
    ...
    public static class MyRestClientErrorHandler implements ResponseErrorHandler {
        ...
        public void handleError(final ClientHttpResponse responseParam) throws IOException {
            if (RestConfig.logger.isInfoEnabled()) {
                RestConfig.logger.info("[ClientConfig.MyRestClientErrorHandler][handleError]-> Error Status code : "
    ...
    public static class MyRestClientInterceptor implements ClientHttpRequestInterceptor {
        ...
        public ClientHttpResponse intercept(final HttpRequest requestParam,
            ...
            ClientHttpResponse executeLoc = executionParam.execute(requestParam,
                                                                   bodyParam);
            if (RestConfig.logger.isInfoEnabled()) {
                RestConfig.logger.info("[ClientConfig.MyRestClientInterceptor][intercept]-> Call Delta : "
    ...
    public RestTemplate restTemplate() {
        ...
        RestTemplate rt = builderLoc.requestFactory(this::clientHttpRequestFactory)
                                    .build();
    ...
    public ConnectionKeepAliveStrategy connectionKeepAliveStrategy() {
        return RestConfig::getKeepAliveDurationProxy;
    }
            ...
            for (Header headerLoc : allHeadersLoc) {
                ...
                if ("Keep-Alive".equals(nameLoc)) {
                    ...
                    if (splitLoc[0].equals("timeout")) {
                        return Long.parseLong(splitLoc[1]) * 1000;
                    }
                }
            }
        } catch (Exception eLoc) {
            RestConfig.logger.error("[ClientConfig][connectionKeepAliveStrategy]-> *Error* : " + eLoc.getMessage(),
                                    eLoc);
        }
        return 60_000;
